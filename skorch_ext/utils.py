...
def train_split_validation_data(X, y, **fit_params):
    X_valid = fit_params.get('X_valid', None)
    y_valid = fit_params.get('y_valid', None)
    if not (isinstance(X_valid, dict) or isinstance(y_valid, numpy.ndarray)): raise ValueError('train_split: train_split_validation_data requires X_valid and y_valid')
    print(X.shape)
    print(X_valid.shape)
