...
class VanillaGrad(object):
    ...
    def __call__(self, x, index=None):
        ...
        output = self.pretrained_model(x=x_var)
        if index is None:
...
class SmoothGrad(VanillaGrad):
    ...
    def __init__(self, pretrained_model, cuda=False, stdev_spread=0.1,
                 n_samples=15, magnitude=True):
    ...
    def __call__(self, x, index=None):
        ...
        for i in range(self.n_samples):
            ...
            output = self.pretrained_model(x=x_plus_noise)
